...
custom_keywords = {
    ...
    '4_cm_introduction': ['from', 'im', 'today', 'line', 'manager', 'care manager', 'care', 'case', 'case manager', 'call', 'assist', 'assist today'],
...
def classify_single_utterance(utterance, keywords, stops):
    ...
    return max(topic_scores, key=topic_scores.get) if max(topic_scores.values()) > 0 else None
