...
custom_keywords = {
    ...
    '2_call_recording_disclosure': ['record', 'call', 'call record', 'purpose', 'service', 'i0'
                                                                                           'nform', 'recorded', 'line', 'aim', 'remind'],
...
def classify_single_utterance(utterance, keywords, stops):
    ...
    return max(topic_scores, key=topic_scores.get) if max(topic_scores.values()) > 0 else 'other_or_no_topic'
